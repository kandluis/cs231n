...
def conv_forward_naive(x, w, b, conv_param):
    ...
    for n in range(N):
        for i in range(newH):
            for j in range(newW):
                sx, sy = i * stride, j * stride
                ex, ey = sx + HH, sy + WW
                sub_image = padded_x[n, :, sx:ex, sy:ey]
                out[n, :, i, j] = np.sum(sub_image * w, axis=(1,2,3)) + b
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    cache = (x, w, b, conv_param)
    ...
    for n in range(N):
        for i in range(newH):
            for j in range(newW):
                ...
                sub_image = padded_x[n, :, sx:ex, sy:ey]
                for f in range(F):
                    dw[f] += sub_image * dout[n, f, i, j]
                    dx_padded[n, :, sx:ex, sy:ey] += w[f] * dout[n, f, i ,j]
...
def max_pool_forward_naive(x, pool_param):
    ...
    N, C, H, W = x.shape
    newH = int(1 + (H - HH) / stride)
    newW = int(1 + (W - WW) / stride)
    ...
    for n in range(N):
        for i in range(newH):
            for j in range(newW):
                sx, sy = i * stride, j * stride
                ex, ey = sx + HH, sy + WW
                sub_image = x[n, :, sx:ex, sy:ey]
                out[n, :, i, j] = np.max(sub_image, axis=(1,2))
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    cache = (x, pool_param)
    ...
    for n in range(N):
        for i in range(newH):
            for j in range(newW):
                ...
                sub_image = x[n, :, sx:ex, sy:ey]
                for c in range(C):
                    k, l = np.unravel_index(np.argmax(
                        sub_image[c]), sub_image[c].shape)
                    dx[n, c, sx + k, sy + l] += dout[n, c, i, j]
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return dx
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    ###########################################################################
    N, C, H, W = x.shape
    reshaped_x = np.reshape(np.swapaxes(x, 1, 3), (-1, C))
    reshaped_out, cache = batchnorm_forward(reshaped_x, gamma, beta, bn_param)
    out = np.swapaxes(np.reshape(reshaped_out, (N, W, H, C)), 1, 3)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def spatial_batchnorm_backward(dout, cache):
    ...
    ###########################################################################
    N, C, H, W = dout.shape
    reshaped_dout = np.reshape(np.swapaxes(dout, 1, 3), (-1, C))
    reshaped_dx, dgamma, dbeta = batchnorm_backward_alt(reshaped_dout, cache)
    dx = np.swapaxes(np.reshape(reshaped_dx, (N, W, H, C)), 1, 3)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
